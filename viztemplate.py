...
{% for bill in bills: -%}
        ...
        {% for tranch_item in tranch -%}
            <div class="coverer
                ...
                {% if ((bill[outer_loop.index0-1][0][1].safe_name == tranch_item[1].safe_name) or (bill[outer_loop.index0-1][-1][1].safe_name == tranch_item[1].safe_name)) and (not outer_loop.first): -%}
                    left_continue 
                {% endif -%}
                {% if (not outer_loop.last) and (bill[outer_loop.index0+1][0][1].safe_name == tranch_item[1].safe_name): -%}
                    right_continue 
                {% endif -%}
    ...
    <div style="clear: both;"></div>
        ...
        <div class="bill-coverer" style="color:white;">
             <div class="text-{{expenditure.safe_name}} name" id="text-{{expenditure.safe_name}}_{{bill_loop.index0}}" style="top: {{expenditure.top}}%; left: {{expenditure.left}}%; width: {{expenditure.text_width}}%;"> 
        ...
        {% for small_bill_amt, small_bill_name in small_bills: -%}
            {% if small_bill_name in exp.bills: -%}
                ...
                <div class="
                    ...
                    <div style="clear: both;"></div>
                    <div class="bill-coverer" style="color:white;">
                         <div class="text-{{exp.safe_name}} name" id="text-{{exp.safe_name}}_{{bill_loop.index0}}" style="top: {{exp.top}}%; left: 0%; width: 100%;"> 
        ...
        {% for small_bill_amt, small_bill_name in coins: -%}
            {% if small_bill_name in exp.bills: -%}
                ...
                <div class="
                    ...
                    <div style="clear: both;"></div>
                    {% if small_bill_name in ["ten", "five", "one"]: -%}
                    <div class="bill-coverer" style="color: white; width: 100%; height: 100%; top: 0%;">
                         <div class="text-{{exp.safe_name}} name" id="text-{{exp.safe_name}}_{{bill_loop.index0}}" style="top: {{exp.top}}%; left: 0%; width: 100%;"> 
                            <span class="expenditurename">{{exp.display_name_split}}</span>
                         </div> 
                    </div> <!-- /bill-coverer -->
